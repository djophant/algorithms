"""This file contains the functions that are used to preprocess the data before it is fed into the model"""

import numpy as np
import gzip
import pickle
import os

os.chdir('/Users/djophant/Downloads/neural-networks-and-deep-learning-master/data')

def load_data():
    """Load the MNIST data
    Returns:
        training_data: list of tuples (x,y) where x is input and y is output
        validation_data: list of tuples (x,y) where x is input and y is output
        test_data: list of tuples (x,y) where x is input and y is output
    """
    with gzip.open('mnist.pkl.gz', 'rb') as f:
        training_data, validation_data, test_data = pickle.load(f, encoding='latin1')
    return training_data, validation_data, test_data

def vectorized_result(j):
    """Return a 10-dimensional unit vector with a 1.0 in the jth position and zeroes elsewhere
    Args:
        j: index
    Returns:
        10-dimensional unit vector
    """
    e = np.zeros((10, 1))
    e[j] = 1.0
    return e

def load_data_wrapper():
    """Load the MNIST data and reshape it
    Returns:
        training_data: list of tuples (x,y) where x is input and y is output
        validation_data: list of tuples (x,y) where x is input and y is output
        test_data: list of tuples (x,y) where x is input and y is output
    """
    tr_d, va_d, te_d = load_data()
    training_inputs = [np.reshape(x, (784, 1)) for x in tr_d[0]]
    training_results = [vectorized_result(y) for y in tr_d[1]]
    training_data = list(zip(training_inputs, training_results))
    validation_inputs = [np.reshape(x, (784, 1)) for x in va_d[0]]
    validation_data = list(zip(validation_inputs, va_d[1]))
    test_inputs = [np.reshape(x, (784, 1)) for x in te_d[0]]
    test_data = list(zip(test_inputs, te_d[1]))
    return training_data, validation_data, test_data